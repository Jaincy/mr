...
public class AppO2oLogSecondDaysRetention extends Configured implements Tool {
	public static void main(String[] args) {
		...
		try {
			ToolRunner.run(new Configuration(),
					new AppO2oLogSecondDaysRetention(), args);
		} catch (Exception e) {
		}
	}
	...
	public int run(String[] args) throws Exception {
		...
		job.setJarByClass(AppO2oLogSecondDaysRetention.class);
		...
		protected void map(Object key, Text value,
			...
			if (count == 2) {
				...
				if (line.indexOf(str1) > 0) {
					...
					context.write(new Text(ID + c + "A" + c + "2"),
							new IntWritable(1));
				}
				...
				if (line.indexOf(str2) > 0 && line.indexOf(str3) > 0) {
					...
					CHL = line.substring(line.indexOf(str3) + 5,
							line.indexOf(str2));
					// key: o2o_id channelno 留存率偏移量（天）
					context.write(new Text(ID + c + CHL + c + "2"),
							new IntWritable(1));
				}
			}
