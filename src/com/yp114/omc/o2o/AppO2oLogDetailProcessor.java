...
public class AppO2oLogDetailProcessor extends Configured implements Tool {
	...
	public static void main(String[] args) {
		...
		try {
			ToolRunner.run(new Configuration(), new AppO2oLogDetailProcessor(), args);
		} catch (Exception e) {
		}
	}
	...
	public int run(String[] args) throws Exception {
		...
		Job job = Job.getInstance(conf, "AppO2oLogDetailProcessor");
		job.setJarByClass(AppO2oLogDetailProcessor.class);
			...
			if (line.length() > 0) {
				...
				if (logidStartIndex > 0 && logidEndIndex > 0) {
					logid = line.substring(logidStartIndex + 8,
							logidEndIndex);
					dayid=line.substring(logidStartIndex + 8,
							logidEndIndex-16);
				}
